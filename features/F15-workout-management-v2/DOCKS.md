...
- Workout cards use a compact category-aware composition: exercise thumbnails, a custom cover image, or a type icon; duration and exercise counts remain visible without opening the workout.
- Stretch workouts show a stretch count rather than set count. Workouts with rep-based exercises show both sets and exercises. Schedule status and resumable state remain secondary but visible.
