...
The offline system supports installed regions and provider-validated layer capabilities. Vector, terrain, imagery, traffic, transit, and other layers may have different legal caching rights; an installed vector region does not imply that every other layer can be cached. The map must show which installed capabilities are available and must not package provider-protected data when the provider does not grant that right.
