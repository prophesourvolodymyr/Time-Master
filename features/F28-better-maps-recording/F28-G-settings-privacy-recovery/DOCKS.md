...
| Maps & Offline | **Offline Maps — Manage** entry | Opens the real installed-region/offline service when that destination is available. It does not fabricate storage totals, tiles, regions, or progress inside the quick pine. | Existing legacy installed-region metadata remains durable. New regions require an explicitly offline-licensed or self-hosted source; MapTiler Cloud configuration alone does not grant offline packaging rights. The map-area selection interior is intentionally deferred as described by F28-D. |
| Maps & Offline | **Auto-download Route Area — Off** | When enabled during an active route session, the service may prepare eligible nearby route data only when the configured source explicitly permits offline packaging and network, storage, and current-route capability allow it. It never blocks recording or reports a download for online-only MapTiler Cloud layers. | New sessions inherit the persisted choice, but the preference remains unavailable until a legally eligible source exists. |
...
| Weather disabled, unavailable, or stale | Weather Info hides the display when disabled. WeatherKit capability, location, network, or refresh failure uses only a temporary response-expiration-bounded cache where legally permitted, otherwise it becomes unavailable; no WeatherKit value is added to durable activity or backup data, and recording continues. |
