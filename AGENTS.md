...
3. If starting: run AUDIT. If continuing: pick up from the last human-reviewed phase.
...
features/FXX-name/DOCKS.md ← Stage 2: Promoted. Solid, approved, ready to build.
    │
    ▼
features/_archive/FXX/     ← Stage 3: Built, human-reviewed, done. Reference only.
```
...
## Dependencies
- FXX-A — must be implemented and human-reviewed before this starts; track the work in `CYCLES.md`
...
You cannot start a feature until the ones it depends on are implemented and human-reviewed.
...
3. Both dependencies (if any) are already implemented and human-reviewed
...
2. All dependencies are already implemented and human-reviewed
3. They do not share files that would cause conflicts
4. Each feature can still be built, handed to the human, and marked implemented separately
5. Their combined scope fits within the 250-550k context budget

AI proposes the bundle to the user:
> "F04 and F05 are small and independent. I can complete both in one ~350k session instead of creating two small prompts. Each will still be implemented and handed to you for review separately. Want to bundle them?"
...
```
DOCKS.md  →  VISUAL PLAN (suggested)  →  AUDIT  →  BUILD  →  CHECKS  →  HUMAN REVIEW
                                                                      │
                                                           ┌──────────┘
                                                           ▼
...
**Phase 5 — Hand off to the human, then connect.** List every implemented feature, sub-feature, and task separately in the final response and state that each is ready for human verification. The human is the final verifier for every feature. Only after human review should dependent work connect. Connecting two unreviewed pieces doesn't fix either one.

### Human Verification Is Mandatory — CRITICAL SYSTEM RULE
The human is the real verifier for every feature the AI implements. AI checks can catch obvious failures, but they do not constitute completion or human approval.

At the end of every implementation session, the AI must:
- List every implemented feature, sub-feature, and meaningful task separately
- State what changed and that each item is ready for human verification
- State any known limitation or unresolved decision plainly
- Mark implementation todos only in `CYCLES.md`

The AI must not claim that a human verified the work. Screenshots, screen recordings, device or OS evidence, logs, and completion evidence packages are not required. DOCKS.md never receives implementation todos, completion checklists, or verification evidence.

**Review Dependencies:** Before starting any feature or sub-feature, re-read its dependency's DOCKS.md and `CYCLES.md`. A dependency must be implemented and handed to the human for review before dependent work starts. AI checks do not replace human review.
...
- Unlimited depth. Every single one gets a DOCKS.md; implementation follow-up belongs only in CYCLES.md.
...
**Human verification rule:** A CYCLES.md `[x]` marks implementation complete only. The human is the final verifier for every implemented feature; AI must list each item in the final handoff and must not mark it human-verified.
...
- Only `[ ]` (pending) and `[x]` (done) — no intermediate states. Implementation todos and human-review handoffs are tracked here; DOCKS.md contains no todos or completion evidence.
...
- **Bundles:** A single prompt may cover several small independent features. CYCLES.md groups them under a bundle, but every feature remains separately implementable, handed off, and checked off.
...
- Treat the human as the final verifier for every implemented feature
- List every implemented feature, sub-feature, and meaningful task separately in the final response for human review
- Update CYCLES.md after implementing a todo — mark it `[x]`; this records implementation, not human verification
...
- Claim that the human verified a feature
- Put todos, completion checklists, screenshots, recordings, logs, or verification evidence in DOCKS.md
- Start dependent work before its dependency has been handed to the human for review
...
| `ready_for_review` | Code written and listed in the final handoff; human review is pending |
| `verified` | Human explicitly confirmed the behavior; AI must not assign this state |
| `done` | Human accepted the implementation and no further work remains |
...
3. Confirm both features' dependencies (if any) have been implemented and human-reviewed
...
Each included feature must be implemented, listed separately for human verification, and checked off in CYCLES.md separately.
...
## Human Verification Handoff — REQUIRED
List every implemented feature, sub-feature, and meaningful task separately. State what changed and that each item is ready for human verification.
- Human is the final verifier for every implemented item
- Do not claim that the work is human-verified
- No screenshots, screen recordings, device or OS evidence, logs, or completion evidence package is required
- Put implementation todos only in `CYCLES.md`; DOCKS.md contains no todos, completion checklists, or verification evidence
...
- Completed: [everything implemented, checked, fixed]
- Remaining: [actual remaining work]
- Next-scope estimate: [Xk context]
- Decision: no next prompt / next prompt created / blocked by user, login, or external dependency
```

### Rules
- Each phase completable in one session
- Sequential — dependent phases only after the human reviews the previous implementation
- No unrelated features combined in one phase
- Every phase ends with a complete human-verification handoff
...
- Update existing DOCKS.md files with the new behavior, states, dependencies, and decisions
- Create new child features only where the improvement needs a separate documented area
- Implement the changes and hand them to the human without pretending this is a clean rebuild
...
- Build the new children from the approved revolution documentation, not from the old agent's assumptions, then hand them to the human

The rebuild agent is the rescue owner. It must inspect the old implementation, identify what is broken or incomplete, and leave the project in a better state ready for human review. It must not blindly preserve weak code or blindly throw away useful behavior.
...
#### Impact Report
Every changed file, archived file, new file, affected feature, affected dependency, branch action, migration step, and review handoff. Include what remains untouched.
...
9. Build the new feature slices, run available checks, fix failures, list every implemented item for human review, and mark implementation todos complete only in CYCLES.md.
10. Record migration notes and deviations in the relevant DOCKS.md files; do not add completion evidence or todos there.
...
- New replacement features are implemented and handed to the human for review
...
- Updating a feature's DOCKS.md with audit findings, implementation facts, or decisions
- Creating feature prompts in `prompts/{feature}/`
- Creating raw ideas and raw feature docs in `genesis/`
- Updating project-specific `STYLES.md` or `features/DOCKS.md`
